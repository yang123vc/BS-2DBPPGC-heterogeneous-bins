...
bool StoppingCriteria(list<NODE>::iterator &father, list<NODE> &tree);
void LastBinRefinement(list<NODE> &BS_tree);

//Main function. This Function creates the whole tree.

void TREE:: build_solution(const char *bin_filename, vector<PIEZA> &pzas)
{
	alpha = 2;
	beta = 3;
    ...
    while(!stop)
		{
            ...
            if(!pzas_avail.empty())
                create_child(bin_filename, No_Childs, No_Rots, *father, pzas_avail, Amax);
            //=====================================
            int nodestot = 0;
            int l = 0;
            while (nodestot<beta)
            ...
            if(next_father->get_level() != father->get_level()) //We are at the end of a level.
            {
                ...
                for(node_level = next_father; node_level != BS_tree.end(); node_level++)
                {
                    node_level->setID(++id);
                }
...
void TREE::del_node(list<NODE>::iterator &nodo)
{
	BS_tree.erase(nodo);
}
...
double Local_of(NODE &node);

bool TREE::local_eval(vector<NODE> &children, NODE &node, int types, double &Amax)
{
    ...
    for(int i= 0; i<node.getPI().size(); i++)
    {
        ...
        ap = ap + pow(p.getArea(),1.1);//Suma de cuadrados de áreas de pzas que están en el bin.
    }
    double area_node = node.getL()*node.getW();
    double node_util = ap/(area_node*Amax);
	...
	for(int i = 0; i<children.size(); i++)
		{
            ...
            if(equal_double(node_util, of_i, TOL))//In case there are nodes with same utilization, break ties with same number of pieces.
            {
...
void KeepBestNodes(NODE &node_eval,vector<GE_COMP>  &best_of, vector<int> &keep_id, vector<int> &no_pzas, int b);
double Construct_Solution(const char *bin_name, vector <PIEZA> pzas, double l, double &Abin, double &Amax);

void TREE::global_eval(const char *bin_filename, vector<PIEZA> &item)
{
...
//            cout<<"\n";
            ...
            if(!pzas_avail.empty())
            {
                of = Construct_Solution(bin_filename,pzas_avail,level, AreaBin, Amax);
                of = AreaPz/of;
	...
	while(ini_level->getID() != node_eval->getID())
		{
		if(find(keep_id.begin(),keep_id.end(),ini_level->getID())== keep_id.end())
			{
...
//		no_pzas.push_back((node_eval.get_IDdisp()).size());
	...
	double b_of = best_of[best_of.size()-1].fractional_util;
    if(node_of.fractional_util >= b_of + PARAM_LAST_BIN)
    {
        is_accepted = true;
    }
    else
    {
        if (node_of.fractional_util >= b_of - PARAM_LAST_BIN && node_of.prop_used_current_bin < best_of[best_of.size() - 1].prop_used_current_bin - TOL2)
        {
            is_accepted = true;
        }
    }
	
    //Insert accepted node in descending order of its utilization
        if(is_accepted)
        {
            int pos  = 0;
            while ((pos<best_of.size()-1 && (node_of.fractional_util <= best_of[pos].fractional_util - PARAM_LAST_BIN)) || (node_of.fractional_util <= best_of[pos].fractional_util + PARAM_LAST_BIN && node_of.prop_used_current_bin > best_of[pos].prop_used_current_bin  && pos<best_of.size() - 1))
                pos++;
...
bool StoppingCriteria(list<NODE>::iterator &node, list<NODE> &tree)
{
	bool stop = false;
