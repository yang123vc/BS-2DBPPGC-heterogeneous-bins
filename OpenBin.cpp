...
vector<double> calculate_rotations_FitBin_Mirror(NODE &b, PIEZA &p, int num_rot, double epsilon);
...
bool PieceInSect(PIEZA &candidate, IRR_BIN &current_sect);
vector<double> calculate_rotations_FitSection_Mirror(vector<EDGES> &sect,PIEZA &p,int num_rot, double epsilon);
bool FitInSect_IniPcFinSec(vector<EDGES>e_pza, vector<PUNTO> ptos_Sec, EDGES esec_match, EDGES epza_match);
bool FitInSect_FinPcIniSec(vector<EDGES>e_pza, vector<PUNTO> ptos_Sec, EDGES esec_match, EDGES epza_match);
int BreakTie(vector<EDGES> ed_pza,vector<PUNTO> ptos_sect, EDGES esec, EDGES epza);
...
void OpenBin(NODE &bin, PIEZA &item)
{
	...
	EDGES base_bin;
	base_bin.ini.coordx = bin.getL();base_bin.ini.coordy = 0.0;
    ...
    {
        cout<<"Piece "<<item.getID() <<" is Bigger than Bin "<< bin.getSize()<<"\n";
    }
    return;
}


bool Place_in_NewSection(IRR_BIN &current_sect, vector<PIEZA*> &candidates,EDGES &best_gc, int id)
{
	bool placed = false;
	...
	if (!placed) current_sect = old_sec;
	return placed;
}

bool PieceInSect(PIEZA &piece, IRR_BIN &current_sect)
{
	if(piece.getArea()>current_sect.getArea()) return false;
	...
	max_rots = 2*max_rots;
//	int max_rots = 2;
	rots = calculate_rotations_FitSection_Mirror(edges_sec,piece,max_rots,ALPHA);
	//===================================================================
	int count_rot = 0;
	...
	while (!placed && count_rot < rots.size())
		...
		for(it_pza = ed_pza.begin();it_pza != ed_pza.end() && !placed; it_pza++)
			{
			for(it_sec =edges_sec.begin(); it_sec != edges_sec.end();it_sec++)
				{
				if(calculate_rotation_match(*it_pza, *it_sec)<=ALPHA)
					{
					//Need to traslate a copy of the edges to see if it would be feasible.
					is_in_sect_IpFs  = FitInSect_IniPcFinSec(ed_pza, ptos_sect,*it_sec, *it_pza);//check feasibility of matching.
					is_in_sect_FpIs = FitInSect_FinPcIniSec(ed_pza, ptos_sect, *it_sec, *it_pza);
					if(is_in_sect_FpIs && is_in_sect_IpFs)//La pieza cabe en ambos extremos
						{
							int pos = BreakTie(ed_pza, ptos_sect, *it_sec, *it_pza);
							if(pos == 0) is_in_sect_FpIs = false;//Aplicamos IniPz-FinSec
							if(pos == 1) is_in_sect_IpFs = false;//APlicamos FinPz-IniSec
...
bool FillBin(vector<PIEZA> &item, IRR_BIN &csect, EDGES &gc,int id, vector<int> ID_disp)
{
	bool placed = false;
	...
	int count = 0;
	while(count < item. size() && item[count].getArea() > csect.getArea()) count++;//Find first piece that may fit in terms of area.
