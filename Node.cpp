...
void NODE::setDim(const char *name, string s)
{
    char name_sol[80];
...
void NODE::initialize_node(const char *filename, int ID, string s)
{
	setDim(filename, s);
...
void NODE::update_section(IRR_BIN &ib)//This function adds the pieces that are included in the irregular bin but not in the section of the bin.
{
	...
	int i = 0;
	while(ib.getID()!= section[i].getID() ) i++;//We are now in the section of the bin that correspond to ib.
	...
	while ((*it_pib)->getID() == (*it_pb)->getID()) {
		it_pib ++;
		it_pb ++;
		if(it_pb == ps->end()) break;//Si llega al final, no sigue
	}
...
void NODE::set_ID_pzas_disp(vector<PIEZA> &pz)
{
	for(int i = 0; i<pz.size(); i++)
		{
		if(!pz[i].is_placed()) ID_pzas_disp.push_back(pz[i].getID());
		}
}

vector<int> NODE::get_IDdisp()
{
	return ID_pzas_disp;
}
...
{
	int pos=0;
	while(pos<ID_pzas_disp.size() && ID_pzas_disp[pos] != ID) pos++;
...
double NODE::get_localevaluation()
{
    return local_eval;
}
