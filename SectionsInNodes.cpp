...
bool FitInSect_IniPcFinSec(vector<EDGES>e_pza, vector<PUNTO> ptos_Sec, EDGES esec_match, EDGES epza_match)
{
	esec_match.set_mod();
	epza_match.set_mod();
	if(esec_match.mod < epza_match.mod) return false;//Edge on the piece is longer than the edge of the section
	...
	for(int i= 0; i<e_pza.size();i++)
		...
		for(int j = 0; j<ptos_Sec.size()-1;j++)
			...
			if(side == 1) return false;//the point is to the left of the section (outside)
			}
...
int BreakTie(vector<EDGES> e_pza,vector<PUNTO> ptos_sect, EDGES esec, EDGES epza)
{
	int best = -1;
	...
	int k = 0;
	while(!equal_pto(ptos_sect[k], esec.fin)) k++;
	int next = k+1;
	if (k+1 >= ptos_sect.size()) next =0;
	...
	k = 0;
	while(!equal_pto(ptos_sect[k], esec.ini)) k++;
	next = k-1;
	if (k-1 < 0) next = ptos_sect.size()-1;
	for(int i = 0; i<e_pza.size(); i++)//For all points in the piece, add those that are in line with the adjacent edge
		{
	...
	//=======================================================
	if(IpFs > FpIs) best = 0; //IniPz-EndSec more matches than the other position
	else best = 1;//EndPz-IniSec more matches than the other position
