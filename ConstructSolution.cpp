...
double SelectBestMatch(IRR_BIN &bin, vector<PIEZA*> list, PIEZA convex_hull, int &best_e1, int &best_e2, double &best_d, int &best_p, int &best_m);
double GuillotineCutWaste(PIEZA ch, IRR_BIN current_sect,EDGES &best_gc);
bool FillBin(vector<PIEZA> &item, IRR_BIN &c_sect, EDGES &best_gc, int i);
...
vector<PIEZA*> CreateList(vector<PIEZA> &item, IRR_BIN &sect)
{
	...
	while(list.size()<LIST_SIZE && p < item.size())
		...
		else p++; //piece does not fit in terms of area, go to next piece.
		}
	return list;
}
...
void RepackLBin(const char *argv, vector<NODE> &bin_sol)
{
    ...
    do
    {
        ...
        if(pz_bin.empty()) continue;//There was only one piece to place.
