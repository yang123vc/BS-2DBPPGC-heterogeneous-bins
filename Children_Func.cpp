...
vector<PIEZA*> CreateList(vector<PIEZA> &item, IRR_BIN &sect, int LIST_SIZE, double thres, vector<int> ID_disp);
...
bool FillBin(vector<PIEZA> &item, IRR_BIN &csect, EDGES &gc, int sectID, vector<int> ID_disp);
bool Place_Candidates(NODE &node, IRR_BIN &sect, vector<PIEZA*> &list, bool &CH_crit, bool &bestCG_crit, bool &NextCG_crit, EDGES &gc, int &sec_mod_id);
bool edge_of_section(EDGES ed, IRR_BIN ib);
EDGES IntersectBin(EDGES gc, vector<PUNTO> rbin);
vector<double> calculate_rotations_FitNode (NODE &b, PIEZA &p, int num_rot, double epsilon);
void fill_node(NODE &current_bin, vector<PIEZA> &pzas);

//Function that creates the children of a node.
void TREE::create_child(const char *argv, int No_Childs, int No_Rots, NODE &father, vector<PIEZA> &pzas_disp, double &Amax)
{
	NODE current_node;
    ...
    for(int t = 0; t<btype.size(); t++)//For each type of bin we create a node with the same initial piece
    {
        ...
        for(int i = 0; i<No_Childs; i++)//For each piece
        {
            ...
            for (int m = 0; m <=1; m++) //For mirror
            {
                ...
                if(m == 1 && is_symetric) continue;
                try_rots = calculate_rotations_FitNode(current_node, pzas_c[i], No_Rots, ALPHA);
                for(int r =0; r<try_rots.size();r++)//For each rotation
                {
                    is_symetric = pzas_c[i].obtener_rotacion(try_rots[r]);
                    if(is_symetric && try_rots[r] != 0) continue;//Rotation does nothing, no point on trying to place it again.
                    if(pzas_c[i].Fits_Bin(current_node.getL(), current_node.getW()))
                    {
                        PIEZA p = pzas_c[i];
...
//    					printf("\nNODE %d:\n Piece %d, \n",current_node.getID(), p.getID());
                        ...
                        is_node_child = local_eval(children, current_node, type_bins, Amax);
                        if(is_node_child)
                        {
                       ...
                       //===================================================
                        ...
                        for(int i = 0; i<children.size();i++)
                        {
                            ...
                            string btype_child = children[i].getSize();
                           if(pza_child.getID() == pza_node.getID() && btype_node.compare(btype_child) == 0)
                            {
                                //If current node has better utilization, delete child
                                if(current_node.get_localevaluation() > children[i].get_localevaluation())
                                {
                        ...
                        if(is_node_child)
                        {
                            ...
                            else
                            {
                                ...
                                int postp = 0;
                                while(postp<children.size() && children[postp].getSize().compare(btype_node)!=0)
                                    postp ++;
                                int pos =postp;
                                double node_util = current_node.getPropUtil();
                                while(pos < children.size() && node_util <= children[pos].getPropUtil()) pos++;
                                current_node.set_level(father.get_level()+1);
                                children.insert(children.begin()+pos, current_node);
...
//					convex_hull = create_rectencl_in_section(current_sect); //Rectangle enclosure for the rectangle instances.
					...
					if(equal_double(convex_hull.getArea(), current_sect.getArea(), TOL)||current_sect.get_waste()<pza_c[last].getArea())//Current section filled up, useless to try it.
						{
					...
					for(int j = 0; j<gc.size(); j++)
						{
						if(edge_of_section(gc[j], current_sect)) continue;
						gc[j] = IntersectBin(gc[j], ptos_sect);
						placed = FillBin(pza_c, current_sect, gc[j], current_bin.getNumSect(), current_bin.get_IDdisp());
						if(placed)
							{
							...
							*ptos_sec = Redefine_IrrBin_withPieces(*ptos_sec, gc[j]);
		...
		candidates = CreateList(pza_c, current_sect, 5,.8,current_bin.get_IDdisp());
		if(candidates.empty())
			{
			int i = 0;
		...
		//Place candidates
		int sec_mod_id = -1;
		placed = Place_Candidates(current_bin, current_sect, candidates, CH_crit, bestCG_crit, NextCG_crit, best_gc, sec_mod_id);
		//==================================
		if(!placed)
			{
			//Place any other piece
			candidates.clear();
			candidates = CreateList(pza_c, current_sect, pzas.size(),0,current_bin.get_IDdisp());
			placed = Place_Candidates(current_bin, current_sect, candidates, CH_crit, bestCG_crit, NextCG_crit, best_gc, sec_mod_id);//Candidate placed in current_sec.
