...
extern vector<double> calculateInnerAngles(vector<EDGES> &aristas);


void EDGES::set_mod()
{
	mod = sqrt(pow(ini.coordx - fin.coordx,2)+pow(ini.coordy-fin.coordy,2));
}
...
bool PIEZA::is_placed() {return placed;}
...
void PIEZA::initialize_piece()
{
	...
	for(int i = 0; i< puntos_angulo_usado.size(); i++)
		{
		if (puntos_angulo_usado[i].coordx>maxX) maxX=puntos_angulo_usado[i].coordx;
		if (puntos_angulo_usado[i].coordx<minX) minX=puntos_angulo_usado[i].coordx;
		if (puntos_angulo_usado[i].coordy>maxY) maxY=puntos_angulo_usado[i].coordy;
		if (puntos_angulo_usado[i].coordy<minY) minY=puntos_angulo_usado[i].coordy;
		}
	//===================================================================
...
vector<PUNTO> *PIEZA::obtener_puntos()
{
...
void PIEZA::ini_edges()
{
	...
	for (int i=0;i< puntos_angulo_usado.size();i++)
		{
		int j=i+1;
		if (j==puntos_angulo_usado.size()) j=0;
		EDGES e;
...
bool PIEZA::obtener_rotacion (double angulo_a_rotar)
{
	...
	for (int i=0;i<n_p;i++)//Bucle para cada uno de los puntos de la pieza.
		...
		double modulo=sqrt(puntos_angulo_inicial[i].coordx*puntos_angulo_inicial[i].coordx + puntos_angulo_inicial[i].coordy*puntos_angulo_inicial[i].coordy);
		double tita=PI/2; // SI x=0 :: y>0
		if (modulo<TOL && modulo>-TOL) tita=0;
		else
			{
			if (puntos_angulo_inicial[i].coordx<TOL && puntos_angulo_inicial[i].coordx>-TOL && puntos_angulo_inicial[i].coordy<-TOL) // SI x=0 :: y<0
				{
				tita=3*PI/2;
				}
			if (puntos_angulo_inicial[i].coordx>TOL && puntos_angulo_inicial[i].coordy>=0) // SI x>0 :: y>=0
				{
				tita=atan(puntos_angulo_inicial[i].coordy/puntos_angulo_inicial[i].coordx);
				}
			if (puntos_angulo_inicial[i].coordx>TOL && puntos_angulo_inicial[i].coordy<-TOL) // SI x>0 :: y<0
				{
				tita=atan(puntos_angulo_inicial[i].coordy/puntos_angulo_inicial[i].coordx) + 2*PI;
				}
			if (puntos_angulo_inicial[i].coordx<-TOL) // SI x<0
				{
				tita=atan(puntos_angulo_inicial[i].coordy/puntos_angulo_inicial[i].coordx) + PI;
				}
			}
		...
		if (ymin>pp.coordy) ymin=pp.coordy;
		if (xmax<pp.coordx) xmax=pp.coordx;
		if (ymax<pp.coordy) ymax=pp.coordy;
		}
...
void PIEZA::write_coords()
{
	...
	for(int i=0; i<no_vertices;i++)
		{
		printf("\t Punto[%d]: ( %.2f , %.2f )\n",i, puntos_angulo_usado[i].coordx, puntos_angulo_usado[i].coordy);
		}
	printf("\n");
}
...
void PIEZA::setbase(vector<EDGES> ar)
{
	...
	for(int i = 0; i<ar.size(); i++)
		{
		if(ar[i].mod >max_edge) max_edge = ar[i].mod;
		}
	base = max_edge;
}
