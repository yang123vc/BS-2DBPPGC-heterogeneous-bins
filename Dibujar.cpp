...
NUMERO redondear_numero_dos_cifras(double num);



void dibujar_nodes (vector<NODE> &node,char *instance, char *name, double t, int escalado)
{
	char filename[80];
	FILE *p_file;
 ...
 int fig = 0;
	...
	for (n=node.begin();n!=node.end();n++)
		...
		if (sc>100/n->getW()) sc=100/n->getW();
		sprintf (cadena,"\\begin{tikzpicture}[scale=0.1, line cap=round, line join=round, >=triangle 45, x=%lf cm,y=%lf cm]",sc,sc);
		fprintf(p_file,"%s \n", cadena);
		cadena[0]='\0';
		
		nn=redondear_numero_dos_cifras(n->getL());
		sprintf (cadena,"\\draw[color=black] (0,0) -- (%d.%d%d,0);",nn.parte_entera,nn.decimal_1,nn.decimal_2);
		fprintf(p_file,"\t %s \n", cadena);
		cadena[0]='\0';
		
		nn=redondear_numero_dos_cifras(n->getW());
		sprintf (cadena,"\\draw[color=black] (0,0) -- (0,%d.%d%d);",nn.parte_entera,nn.decimal_1,nn.decimal_2);
		fprintf(p_file,"\t %s \n", cadena);
		cadena[0]='\0';
		...
		sprintf (cadena,"\\clip(-3,-3) rectangle (%d.%d%d,%d.%d%d);",nn.parte_entera,nn.decimal_1,nn.decimal_2,nn1.parte_entera,nn1.decimal_1,nn1.decimal_2);
		fprintf(p_file,"\t %s \n", cadena);
		cadena[0]='\0';
		...
		sprintf (cadena,"\\draw[color=black] (-2,%d.%d%d) -- (%d.%d%d,%d.%d%d);",nn.parte_entera,nn.decimal_1,nn.decimal_2,
				 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2,nn2.parte_entera,nn2.decimal_1,nn2.decimal_2);
		fprintf(p_file,"\t %s \n", cadena);
		cadena[0]='\0';
		...
		nn1=redondear_numero_dos_cifras(n->getL());
		nn2=redondear_numero_dos_cifras(n->getW());
		sprintf (cadena,"\\draw[color=black] (%d.%d%d,-2) -- (%d.%d%d,%d.%d%d);",nn.parte_entera,nn.decimal_1,nn.decimal_2,
				 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2,nn2.parte_entera,nn2.decimal_1,nn2.decimal_2);
		fprintf(p_file,"\t %s \n", cadena);
		cadena[0]='\0';
		...
		for (int i=0;i<n->getNumPiezas();i++)
			...
			fprintf(p_file,"%s \n", cadena8);
			cadena8[0]='\0';
			nn = redondear_numero_dos_cifras(pb[i]->obtener_puntos()->front().coordx);
			nn1 = redondear_numero_dos_cifras(pb[i]->obtener_puntos()->front().coordy);
			
			sprintf (cadena8,"\\fill[color=zzttqq,fill=zzttqq,fill opacity=0.2] (%d.%d%d,%d.%d%d)",nn.parte_entera,nn.decimal_1,nn.decimal_2,
					 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2);
			
			for (int j=1;j<pb[i]->obtener_puntos()->size();j++)
				{
				nn = redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[j - 1].coordx);
				nn1 = redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[j - 1].coordy);
				nn2 = redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[j].coordx);
				nn3 = redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[j].coordy);
				
				char aux[30];
				char cadena9[120];
				sprintf (cadena9,"\\draw[color=zzttqq] (%d.%d%d,%d.%d%d) -- (%d.%d%d,%d.%d%d);",nn.parte_entera,nn.decimal_1,nn.decimal_2,
						 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2,nn2.parte_entera,nn2.decimal_1,nn2.decimal_2,nn3.parte_entera,nn3.decimal_1,nn3.decimal_2);
				fprintf(p_file,"\t %s \n", cadena9);
				cadena9[0]='\0';
				nn=redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[j].coordx);
				nn1=redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[j].coordy);
				
				sprintf (aux," -- (%d.%d%d,%d.%d%d)",nn.parte_entera,nn.decimal_1,nn.decimal_2,
						 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2);
				strcat (cadena8,aux);
				aux[0]='\0';
				}
			
			nn = redondear_numero_dos_cifras((*pb[i]->obtener_puntos()).back().coordx);
			nn1 = redondear_numero_dos_cifras((*pb[i]->obtener_puntos()).back().coordy);
			nn2 = redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[0].coordx);
			nn3 = redondear_numero_dos_cifras((*pb[i]->obtener_puntos())[0].coordy);
			char cadena9[1200];
			sprintf (cadena9,"\\draw[color=zzttqq] (%d.%d%d,%d.%d%d) -- (%d.%d%d,%d.%d%d);",nn.parte_entera,nn.decimal_1,nn.decimal_2,
					 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2,nn2.parte_entera,nn2.decimal_1,nn2.decimal_2,nn3.parte_entera,nn3.decimal_1,nn3.decimal_2);
			fprintf(p_file,"\t %s \n", cadena9);
			...
			nn=redondear_numero_dos_cifras(pb[i]->getCoord().coordx+(pb[i]->getl()/2));
			nn1=redondear_numero_dos_cifras(pb[i]->getCoord().coordy+(pb[i]->getw()/2));
			sprintf (cadena8,"\\draw (%d.%d%d,%d.%d%d) node {\\tiny $%d$}; ",nn.parte_entera,nn.decimal_1,nn.decimal_2,
					 nn1.parte_entera,nn1.decimal_1,nn1.decimal_2, pb[i]->getID());
			fprintf(p_file,"\t %s \n", cadena8);
			cadena8[0]='\0';
			}
		...
		for (gc = (*gil_cut).begin(); gc != (*gil_cut).end(); gc++) // para cada corte guillotina
			...
			sprintf(cadena9, "\\draw[color=black] (%d.%d%d,%d.%d%d) -- (%d.%d%d,%d.%d%d);", nn.parte_entera, nn.decimal_1, nn.decimal_2,
					nn1.parte_entera, nn1.decimal_1, nn1.decimal_2, nn2.parte_entera, nn2.decimal_1, nn2.decimal_2, nn3.parte_entera, nn3.decimal_1, nn3.decimal_2);
			fprintf(p_file, "%s  \n", cadena9);
			cadena9[0] = '\0';
		...
		sprintf (cadena,"\\caption{BIN %d - Size %s- USAGE %2lf}",n->getID(),n->getSize().c_str(), n->getPropUtil());
