...
double SelectBestMatch(IRR_BIN &bin, vector<PIEZA*> list, PIEZA ch, int &b_e1, int &b_e2, double &b_d, int &b_p, int &b_m);
...
bool Place_Candidates(NODE &node, IRR_BIN &sect, vector<PIEZA*> &list, bool &CH_crit, bool &bestGC_crit, bool &NextGC_crit, EDGES &best_gc, int &sec_mod_id)
{
	bool placed = false;
	PIEZA convex_hull;
	...
	ch_util = SelectBestMatch(sect, list, convex_hull, best_i, best_j, best_d, best_p, best_m);
	...
	if(ch_util != -1 && ch_util > gc_util)
		...
		Feasible_Slide_InSect(sect,convex_hull, *list[best_p], best_i, best_j, best_d);//Slide edges along best distance
...
//			convex_hull = create_rectencl_in_section(sect); //Rectangle enclosure for the rectangle instances.
			...
			if(equal_pto(best_gc.ini, gc_empty) && equal_pto(best_gc.fin, gc_empty))//No more gc possible in current section, move to next
				continue;
