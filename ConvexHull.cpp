...
bool lexicographic(PUNTO A, PUNTO B);
// 2D cross product of OA and OB vectors, i.e. z-component of their 3D cross product.
// Returns a positive value, if OAB makes a counter-clockwise turn,
// negative for clockwise turn, and zero if the points are collinear.
double cross(PUNTO &O, PUNTO &A, PUNTO &B)
{
	return (A.coordx - O.coordx) * (B.coordy - O.coordy) - (A.coordy - O.coordy) * (B.coordx - O.coordx);
}
...
double convexhull_Height(PIEZA P1, PIEZA P2)
{
	...
	for(int i = 0; i<ch.size();i++)
		{
		if(ch[i].coordy>alto) alto = ch[i].coordy;
		}
	//=========================================
	return alto;
}
...
double convexhull_Length(PIEZA P1, PIEZA P2)
{
	...
	for(int i = 0; i<ch.size();i++)
		{
		if(ch[i].coordx>ancho) ancho = ch[i].coordx;
		}
	//=========================================
	return ancho;
}
