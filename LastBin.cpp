...
//					PIEZA ch = create_rectencl_in_section(sec); //Rectangle enclosure for the rectangle instances.
					...
					if(equal_double(maxheight, it_node->getW(), TOL)) break;
					}
				hor_util = maxheight/it_node->getW();
				if(hor_util<0) hor_util = 1.0;
				}
            else
            {
...
//					PIEZA ch = create_rectencl_in_section(sec); //Rectangle enclosure for the rectangle instances.
					...
					if(equal_double(maxlength, it_node->getL(), TOL)) break;
					}
				ver_util = maxlength/it_node->getL();
				if(ver_util<0) ver_util = 1.0;
				}
            else
            {
