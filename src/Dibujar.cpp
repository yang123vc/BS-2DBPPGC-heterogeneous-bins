...
void dibujar_nodes(vector<NODE> &node, const string &instance,
		const string &name, double t, double usage, int escalado) {
	FILE *p_file;
...
//  strcat (filename, name_inst);
	...
	const string filename = name + name_inst + ".tex";

	if ((p_file = fopen(filename.c_str(), "w")) == NULL) /*  abre el fichero .txt  */
	{
