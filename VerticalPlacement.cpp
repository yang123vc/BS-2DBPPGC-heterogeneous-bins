...
double HorGuillCut(IRR_BIN &ib_node, vector<PIEZA*> last_pzas, PIEZA convex_hull, double nodeW, double nodeL, double &best_rot, int &best_p, int &best_i, EDGES &vcg);
void trans_piece_ini_fin(PIEZA &item2, EDGES e1, EDGES e2);
double rectencl_Length(PIEZA P1, PIEZA P2);
...
double ArrangeVert(vector<PIEZA> &pzas, NODE &node)
{
	...
	//==========================
	EDGES side_bin;
	side_bin.ini.coordx = 0.0; side_bin.ini.coordy = 0.0;
	side_bin.fin.coordx = 0.0;side_bin.fin.coordy = nodeW;
	side_bin.set_mod();
	double rot_angle = -1;
	...
	while (!last_pzas.empty())//Not all the pieces have been placed.
		...
		length_bm = VertAligMatch(ib_node, last_pzas, convex_hull, best_ec, best_ep, best_d, best_p_bm, best_m);
		if(length_gc < GRANDE)
			length_gc = HorGuillCut(ib_node, last_pzas, convex_hull, nodeW, nodeL, best_rot, best_p_hc, best_i_hc, hcg);//Devuelve la altura total si la pieza se coloca a la dcha del actual convex hull.
		if (length_bm == GRANDE && length_gc == GRANDE) return vert_util; //Piece did not fit
						...
						if(md>0-TOL)
							...
							for(double d = 0; d<=dist; d+=slide_ep)
								...
								if(slide_feas)//Slide edge j from piece to edge i of convex hull only if feasible
									{
									double Length = convexhull_Length(convex_hull, citem);
...
double HorGuillCut(IRR_BIN &ib_node, vector<PIEZA*> list, PIEZA convex_hull, double node_w, double node_l, double &best_rot, int &best_p, int &best_i, EDGES &h_gc)
{
	double MaxLength = GRANDE;
	PIEZA item;
	double xmin = GRANDE;
	...
	h_gc.fin.coordx = xmin; h_gc.fin.coordy = ymax;
	h_gc.ini.coordx = node_l; h_gc.ini.coordy = ymax;
	h_gc.set_mod();
