...
bool FitInSect(vector<EDGES>rp, vector<PUNTO> ptos_Sec);
...
double calculate_rotation_match (EDGES &e1,EDGES &e2)
{
	...
	if (cos_alpha>=1-TOL2)//Tolerance problems if cos_alpha is close to 1, alpha = 0.
		...
		{
		if(equal_double(cos_alpha, -1.0, TOL)) cos_alpha = -1.0; //Avoid problems if cos_alpha very close to -1.
...
vector<EDGES> rotation_piece (PIEZA &p, double alpha)
{
	...
	for (it=pp->begin();it!=pp->end();it++)
		{
		double modulo=sqrt((*it).coordx*(*it).coordx + (*it).coordy*(*it).coordy);
...
vector<double> calculate_rotations_FitSection_Mirror(vector<EDGES> &ed_irrbin,PIEZA &p,int num_rot, double epsilon)
{
...
//	for(int m = 0; m <= 0; m++)//For the rectangle instances, no need to mirror pieces
	...
	for(int m = 0; m <= 1; m++)
		{
		if(m == 1 && p.get_mirror()==false) p.set_mirror(true);//Trabajamos con la pieza reflejada.
	for (it_ep=edges_p->begin();it_ep!=edges_p->end();it_ep++)
		...
		for (it_es=ed_irrbin.begin();it_es!=ed_irrbin.end();it_es++)
			...
			if(m == 1)
				{
				if(equal_double(alpha, 0, epsilon)) alpha_mir = -1000;
				else alpha_mir = -1*alpha;
				}
			
			bool ya_probado=false;
			for (int compr_disp=0;compr_disp<num_rot;compr_disp++)
				{
				if (alpha_mir>=g_angles[compr_disp]-epsilon && alpha_mir<=g_angles[compr_disp]+epsilon)
					{
					ya_probado=true;
					break;
					}
				}
			if (ya_probado) continue;
			...
			for (it_e3=rp.begin();it_e3!=rp.end();it_e3++)
				{
				for (it_e4=ed_irrbin.begin();it_e4!=ed_irrbin.end();it_e4++)
					{
					(*it_e3).set_mod();
					if (calculate_rotation_match ((*it_e3),(*it_e4))<=epsilon)
						...
						if (aux_d<min_length) min_length=aux_d;
			...
			int pos=0;
			while (pos<g_matchings.size() && (cont_match<g_matchings[pos] || (cont_match==g_matchings[pos] && bt2<breaking_ties[pos]-TOL)))
				{
				pos++;
				}
			...
			if (pos<g_matchings.size())
				...
				if(m == 0)g_angles[pos]=alpha;//Piece is not mirrored
				else if (alpha == 0) g_angles[pos] = -1000;//Piece is mirrored and rotation angle is 0.
				else g_angles[pos] = -1 * alpha;//piece is mirrored.
    ...
    for(int m = 0; m<= 1; m++)
    {
        ...
        for (it_e=edges_p->begin();it_e!=edges_p->end();it_e++)
        {
            for (it_e2=edges_bin.begin();it_e2!=edges_bin.end();it_e2++)
            {
                ...
                for (it_e3=rp.begin();it_e3!=rp.end();it_e3++)
                {
                    for (it_e4=edges_bin.begin();it_e4!=edges_bin.end();it_e4++)
                    {
                        if (calculate_rotation_match ((*it_e3),(*it_e4))<=epsilon)
                        {
                            cont_match++;
...
vector<double> calculate_rotations_FitNode (NODE &n, PIEZA &p, int num_rot, double epsilon)
{
	...
	for (it_e=edges_p->begin();it_e!=edges_p->end();it_e++)
		...
		for (it_e2=edges_bin.begin();it_e2!=edges_bin.end();it_e2++)
			...
			for (int compr_disp=0;compr_disp<num_rot;compr_disp++)
				{
				if (alpha>=g_angles[compr_disp]-epsilon && alpha<=g_angles[compr_disp]+epsilon)
					{
					ya_probado=true;
					break;
					}
