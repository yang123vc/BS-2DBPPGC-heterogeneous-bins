...
vector<PIEZA> read_pieces(char* name)
{
	...
	while(!p_file.eof())//Mientras no llegue al fin del archivo (end_of_file)
		...
		if (pieza_anterior==-1) pieza_anterior=pieza;
...
{
	if(a >= b-prec && a <= b+prec) return true;
	else return false;
}

bool equal_pto (PUNTO a, PUNTO b)
{
	if(equal_double(a.coordx, b.coordx, TOL) && equal_double(a.coordy, b.coordy, TOL)) return true;
	else return false;
}
...
int Position(PUNTO A, PUNTO B, PUNTO C)
{
	...
	if(fabs(B.coordx-C.coordx)<0.01 && fabs(B.coordy-C.coordy)<0.01)//B & C can be considered the same point
		C = B;
	//=============================================================
	crosprod = (B.coordx - A.coordx)*(C.coordy - A.coordy) - (B.coordy - A.coordy)*(C.coordx - A.coordx);
	if (crosprod > 0+TOL) return 1;//C is left of vector AB
	if (crosprod < 0-TOL) return -1;//C is right of vector AB
	if (equal_double(crosprod, 0, TOL)) return 0;//Alignded
...
bool right_angles(vector<double> angle)
{
	...
	while(count <angle.size() && !has_right)
		{
		if(equal_double(angle[count], 90, ALPHA)) has_right = true;
		count ++;
		}
...
{
	return (P1.coordx < P2.coordx || (P1.coordx == P2.coordx && P1.coordy < P2.coordy));
}
